...
TEST_F(IBitStreamTest, testReadBit)
{
    ...
    EXPECT_EQ(0, static_cast<IBitStream&>(*istream_).getReadBitCount()) <<
        "read bit count";
    EXPECT_EQ(8, istream_->getTotalBitCount()) << "total bit count";
    EXPECT_EQ(1, istream_->getTotalSize()) << "total byte";

    UInt8 bit;
    istream_->read(bit, 1);
    EXPECT_EQ(1, bit) << "bit";
    EXPECT_EQ(1, static_cast<IBitStream&>(*istream_).getReadBitCount()) <<
        "read bit count";
    EXPECT_EQ(8, istream_->getTotalBitCount()) << "total bit count";
    EXPECT_EQ(1, istream_->getTotalSize()) << "total byte";
}
...
TEST_F(IBitStreamTest, testBitsNotEnough)
{
    ...
    EXPECT_THROW(istream_->read(value, 2), Exception);
}
...
TEST_F(IBitStreamTest, testReadSignedInt)
{
    ...
    EXPECT_EQ(ostream_->getTotalSize(), istream_->getTotalSize());
...
TEST_F(IBitStreamTest, testReadZeroLengthString)
{
    String original("");
    ostream_->write(original, USHRT_MAX, Bits<UInt16>::size);
    EXPECT_EQ(2 + 0, ostream_->getTotalSize()) << "2 + 0 bytes";
...
TEST_F(IBitStreamTest, testReadUInt64)
{
    ...
    EXPECT_EQ(64 * 10, istream_->getTotalBitCount());
    EXPECT_EQ(80, istream_->getTotalSize());

    UInt64 value;
    for (UInt64 i = 0; i < 10; ++i) {
        istream_->read(value);
        EXPECT_EQ(i + INT_MAX, value) << "#" << i << " value";
        EXPECT_EQ(0, static_cast<IBitStream&>(*istream_).getReadBitCount());
    }
}
